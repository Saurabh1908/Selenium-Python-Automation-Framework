...
worksheet = openpyxl.load_workbook("../TestData/testdata.xlsx")
...
class ExcelMethods(BaseData):
    def get_parameters(self):
        self.delete_existing_result_column()
        # sheet = worksheet['test_Login']
        parametrize_list= list(self.sheet.values)[1:]
        print(parametrize_list)
        return parametrize_list


    def update_result_in_excel(self,sr_no,status):
        row_number = 0
        ...
        worksheet.save("../TestData/testdata.xlsx")
    ...
    def delete_existing_result_column(self):
        # sheet = worksheet['test_Login']
        list_with_values = []
        for cell in self.sheet[1]:
            list_with_values.append(cell.value)
